import cv2
import numpy as np
import glob
import os.path as osp
import os
import yaml
import torch
from pathlib import Path
from multiprocessing import Process, Queue
from plyfile import PlyElement, PlyData

from odometry.dpvo.utils import Timer
from odometry.dpvo.dpvo import DPVO
from odometry.dpvo.config import cfg
from odometry.dpvo.stream import image_stream, video_stream, realsense_stream, camera_stream
from odometry.dpvo.plot_utils import plot_trajectory, save_trajectory_tum_format
from cameras.realsense import init_rs_camera

SKIP = 0

def show_image(image, t=0):
    image = image.permute(1, 2, 0).cpu().numpy()
    cv2.imshow('image', image / 255.0)
    cv2.waitKey(t)

@torch.no_grad()
def run(cfg, network, imagedir, calib, stride=1, skip=0, camera_input=False, viz=False, timeit=False, save_reconstruction=False, output_name='mustard0'):

    slam = None
    queue = Queue(maxsize=8)
    if camera_input:
        # width = 640
        # height = 480
        # (
        #     pipeline, 
        #     align, 
        #     _, 
        #     _, 
        #     depth_scale_meters, 
        #     _, 
        #     _ 
        # ) = init_rs_camera(width, height)
        camera_idx = 6
        reader = Process(target=camera_stream, args=(queue, camera_idx, calib, stride, skip))
    else:
        print(imagedir)
        if os.path.isdir(imagedir):
            reader = Process(target=image_stream, args=(queue, imagedir, calib, stride, skip))
        else:
            reader = Process(target=video_stream, args=(queue, imagedir, calib, stride, skip))

    reader.start()

    while 1:
        (t, image, intrinsics) = queue.get()
        if t < 0: break

        image = torch.from_numpy(image).permute(2,0,1).to(torch.device('cuda:0'))
        intrinsics = torch.from_numpy(intrinsics).to(torch.device('cuda:0'))

        if slam is None:
            slam = DPVO(cfg, network, device=torch.device('cuda:0'), ht=image.shape[1], wd=image.shape[2], viz=viz)

        image = image.to(torch.device('cuda:0'))
        intrinsics = intrinsics.to(torch.device('cuda:0'))

        with Timer("SLAM", enabled=timeit):
            print("DEBUG: Entering update step")
            slam(t, image, intrinsics)

    for _ in range(12):
        slam.update()

    reader.join()

    print("Dumping poses and patch depths...")
    slam.dump_poses(output_name)
    slam.dump_patch_depths(output_name)

    if save_reconstruction:
        points = slam.points_.cpu().numpy()[:slam.m]
        colors = slam.colors_.view(-1, 3).cpu().numpy()[:slam.m]
        points = np.array([(x,y,z,r,g,b) for (x,y,z),(r,g,b) in zip(points, colors)],
                          dtype=[('x', '<f4'), ('y', '<f4'), ('z', '<f4'),('red', 'u1'), ('green', 'u1'),('blue', 'u1')])
        el = PlyElement.describe(points, 'vertex',{'some_property': 'f8'},{'some_property': 'u4'})
        return slam.terminate(), PlyData([el], text=True)

    return slam.terminate()

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--network', type=str, default='dpvo.pth')
    parser.add_argument('--imagedir', type=str)
    parser.add_argument('--camera_stream', action="store_true")
    parser.add_argument('--stride', type=int, default=2)
    parser.add_argument('--skip', type=int, default=0)
    parser.add_argument('--buffer', type=int, default=2048)
    parser.add_argument('--timeit', action='store_true')
    parser.add_argument('--viz', action="store_true")
    parser.add_argument('--plot', action="store_true")
    parser.add_argument('--output_name', type=str, default='mustard0')
    parser.add_argument('--save_reconstruction', action="store_true")
    parser.add_argument('--save_trajectory', action="store_true")

    parser.set_defaults(camera_stream=False)

    # NOTE: viz is currently disabled because I need to find how to make Pangolin work on GPU.
    # Currently, I am not able to render the DPVO sparse PCD on GPU, but I need to move patches on CPU, 
    # making the whole visualization process slower.
    parser.set_defaults(viz=False)
    parser.set_defaults(save_reconstruction=False)
    parser.set_defaults(save_trajectory=False)

    args = parser.parse_args()

    with open("src/yarp-app/configs/default.yaml", "r") as cfg_file:
        input_cfg = yaml.safe_load(cfg_file)

    opts = []
    for key, value in input_cfg['odometry']['dpvo_config'].items():
        if key in cfg:
            opts.append(key)
            opts.append(value)

    cfg.merge_from_list(opts)
    cfg.BUFFER_SIZE = args.buffer

    print("Running on device: ", torch.cuda.get_device_name(0))
    print("Running with config...")
    print(cfg)

    args.network = os.path.join(input_cfg['odometry']['model_ckpts_path'], args.network)
    args.calib = input_cfg['global']['calib']

    pred_traj = run(cfg, 
                    args.network, 
                    args.imagedir, 
                    args.calib, 
                    args.stride, 
                    args.skip,
                    args.camera_stream, 
                    args.viz, 
                    args.timeit, 
                    args.save_reconstruction,
                    args.output_name)

    name = Path(args.imagedir).stem

    if args.save_reconstruction:
        pred_traj, ply_data = pred_traj
        ply_data.write(f"{name}.ply")
        print(f"Saved {name}.ply")

    if args.save_trajectory:
        Path("saved_trajectories").mkdir(exist_ok=True)
        save_trajectory_tum_format(pred_traj, f"saved_trajectories/{name}.txt")

    if args.plot:
        Path("trajectory_plots").mkdir(exist_ok=True)
        plot_trajectory(pred_traj, title=f"DPVO Trajectory Prediction for {name}", filename=f"trajectory_plots/{name}.pdf")


        

