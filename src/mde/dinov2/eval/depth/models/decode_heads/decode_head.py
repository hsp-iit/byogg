...
class DepthBaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        print(f"[NOTE] self.scale_up = {self.scale_up}")
    ...
    def forward_train(self, img, inputs, img_metas, depth_gt, train_cfg):
        ...
        losses = self.losses(depth_pred, depth_gt, img_metas)
    ...
    def losses(self, depth_pred, depth_gt, img_metas):
        """Compute depth loss."""
        loss = dict()
        depth_pred = resize(
        ...
        for i in range(depth_pred.shape[0]):
            for loss_decode in losses_decode:
                if loss_decode.loss_name not in loss:
                    loss[loss_decode.loss_name] = loss_decode(depth_pred[i].squeeze(), depth_gt[i].squeeze(), img_metas[i])
                else:
                    loss[loss_decode.loss_name] += loss_decode(depth_pred[i].squeeze(), depth_gt[i].squeeze(), img_metas[i])

        for loss_name in loss:
            loss[loss_name] /= depth_pred.shape[0]
    ...
    def log_images(self, img_path, depth_pred, depth_gt, img_meta):
        ...
        # NOTE: Normalization is done outside this function.
