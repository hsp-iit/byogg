...
class ConvModule(nn.Module):
    ...
    def __init__(
        ...
        if self.with_norm:
            ...
            self.norm_layer = build_norm_layer(self.norm_cfg, norm_channels)
            self.add_module("norm", self.norm_layer)

            if self.with_bias:
                from torch.nn.modules.batchnorm import _BatchNorm
                from torch.nn.modules.instancenorm import _InstanceNorm

                if isinstance(self.norm_layer, (_BatchNorm, _InstanceNorm)):
        ...
        if self.with_activation:
            ...
            if self.act_cfg['type'] not in ['Tanh', 'PReLU', 'Sigmoid', 'GELU']:
                self.act_cfg['inplace'] = self.inplace

            self.act_layer = build_activation_layer(self.act_cfg)
...
class DPTHead(DepthBaseDecodeHead):
    ...
    def forward(self, inputs, img_metas):
        ...
        out = self.project(out) # after this projection there is a ReLU which may cause problems in learning
