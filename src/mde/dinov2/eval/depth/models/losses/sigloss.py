...
class SigLoss(nn.Module):
    ...
    def __init__(
        self, valid_mask=True, loss_weight=1.0, max_depth=None, warm_up=False, warm_iter=100, loss_name="sigloss", scale_inv_weight=0.75
    ):
    ...
    def sigloss(self, input, target):
        ...
        if self.warm_up:
            if self.warm_up_counter < self.warm_iter:
                ...
                g = (1 - self.scale_inv_weight) * torch.pow(torch.mean(g), 2)
                self.warm_up_counter += 1
                return torch.sqrt(g)

        g = torch.log(input + self.eps) - torch.log(target + self.eps)
        Dg = torch.var(g) + (1 - self.scale_inv_weight) * torch.pow(torch.mean(g), 2)
