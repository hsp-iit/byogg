...
from grasping.contact_graspnet.utils.tf_train_ops import get_bn_decay
import grasping.contact_graspnet.utils.config_utils as config_utils
from grasping.contact_graspnet.utils.data_utils import farthest_points, distance_by_translation_point, preprocess_pc_for_inference, regularize_pc_point_count, depth2pc, reject_median_outliers
...
class GraspEstimator:
    ...
    def predict_scene_grasps(self, sess, pc_full, pc_segments={}, local_regions=False, filter_grasps=False, forward_passes=1, sampling_strategy='random'):
        """
        ...
        if local_regions:
            ...
            # print(self._contact_grasp_cfg['DATA'])
            # print(self._contact_grasp_cfg['DATA']['raw_num_points'])
            pc_full = regularize_pc_point_count(pc_full, self._contact_grasp_cfg['DATA']['raw_num_points'], sampling_strategy=sampling_strategy)
    ...
    def extract_point_clouds(self, depth, K, segmap=None, rgb=None, 
                             z_range=[0.2, 1.0], segmap_id=0, skip_border_objects=False, margin_px=5, 
        ...
        print(f'pc_full.shape: {pc_full.shape}')
        ...
        print(f'pc_full.shape after distance thresholding: {pc_full.shape}')
