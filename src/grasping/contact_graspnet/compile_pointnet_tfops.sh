INCLUDE='-I/root/miniconda3/envs/byogg/include'
...
nvcc -std=c++11 -c -o tf_sampling_g.cu.o tf_sampling_g.cu \
 ${INCLUDE} ${CUDA_INCLUDE} ${TF_CFLAGS} -D GOOGLE_CUDA=1 -x cu -Xcompiler -fPIC

g++ -std=c++11 -shared -o tf_sampling_so.so tf_sampling.cpp \
 tf_sampling_g.cu.o ${INCLUDE} ${CUDA_INCLUDE} ${TF_CFLAGS} -fPIC -lcudart ${TF_LFLAGS} ${CUDA_LIB}
...
g++ -std=c++11 -shared -o tf_grouping_so.so tf_grouping.cpp \
 tf_grouping_g.cu.o ${INCLUDE} ${CUDA_INCLUDE} ${TF_CFLAGS} -fPIC -lcudart ${TF_LFLAGS} ${CUDA_LIB}
...
cd ../3d_interpolation
g++ -std=c++11 tf_interpolate.cpp -o tf_interpolate_so.so ${INCLUDE} -shared -fPIC -shared -fPIC ${TF_CFLAGS} ${TF_LFLAGS} -O2
