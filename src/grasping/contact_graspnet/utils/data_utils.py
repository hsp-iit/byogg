...
BASE_DIR = os.path.join(os.environ['PYTHONPATH'], 'src')
...
def regularize_pc_point_count(pc, npoints, use_farthest_point=False, sampling_strategy='random'):
    """
    ...
    """
    assert sampling_strategy in ['random', 'inv-depth-mass', 'farthest-point']
    # NOTE: A sampling strategy is only used when downsampling. 
    print(f'Using sampling strategy {sampling_strategy} to downsample pc.')
    if pc.shape[0] > npoints:
        if use_farthest_point or sampling_strategy == 'farthest-point':
            ...
            elif sampling_strategy == 'inv-depth-mass':
                invdepths = 1 / pc [:, 2]
                probs = softmax(invdepths)
                center_indexes = np.random.choice(range(pc.shape[0]), size=npoints, replace=False, p=probs)
